...
def run(init_lr=0.1,
    ...
    print(f"Number of Samples: {len(dataloaders['test'])}")
    ...
    top1_fp = np.full(num_classes, 0.000001, dtype=float) 
    # print(top1_fp)   
    top1_tp = np.zeros(num_classes, dtype=int)

    top5_fp = np.full(num_classes, 0.000001, dtype=float)   
    top5_tp = np.zeros(num_classes, dtype=int)

    top10_fp = np.full(num_classes, 0.000001, dtype=float)   
    ...
    print(f'top-k average per class acc: {top1_per_class}, {top5_per_class}, {top10_per_class}')
...
def ensemble(mode, root, train_split, weights, num_classes):
    ...
    for i, data in enumerate(dataloaders["test"]):
        print(f"Number of Samples: {i+1}/{len(dataloaders['test'])}")
        inputs, labels, video_id = data  # inputs: b, c, t, h, w
...
if __name__ == '__main__':
    ...
    root = '/users/PDNU0015/trann2/WLASL/data/WLASL2000'
